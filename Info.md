...
### Cleaning up cache versions:
the best place to clean is in activated SW listener, SW will not be activated till all tab are closed, and the the old cache not in use so we can clean it up:
```
// sw.js
